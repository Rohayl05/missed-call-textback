...
FastAPI webhook service for UK trades businesses. Two features:
1. **Missed-call auto text-back** — when a call to your Twilio number goes unanswered, the caller gets an SMS within seconds.
2. **Review request automation** — trigger a "job complete" event and the customer receives a review-request SMS 2 hours later.
...
The review SMS will be sent 2 hours later (adjust `REVIEW_DELAY_SECONDS` in `config.py` for testing).
...
| POST | `/job-complete` | Schedules review SMS 2 hrs later (`customer_number`, `customer_name` form fields) |
