...
def dashboard(bookings: list, missed_calls: list) -> str:
    ...
    if missed:
        missed_html = "".join(
            f"""<div class="item"><strong>{escape(c['caller'])}</strong>
              <div class="meta">{_fmt(c.get('timestamp'))} UTC</div></div>"""
            for c in missed
        )
    else:
        missed_html = '<div class="empty">No missed calls.</div>'
    ...
    <div class="card"><h2>Missed calls</h2>{missed_html}</div>"""
